...
class Net:
    ...
    def post(self, url, **kw):
        ...
        for i in range(RETRIES):
            try:
                return self.s.post(url, timeout=TIMEOUT, **kw)
            except Exception as e:
                last = e
                if i < RETRIES - 1:
                    time.sleep(RETRY_WAIT * (i + 1))
...
def download_item_pdf(net, item_id, out_dir, limit=DEFAULT_LIMIT, force=False, idx=None, total=None):
    ...
    if ok and parts:
        ...
        try:
            ...
            pass
        log(f"  ✓ 已保存（合并 {len(parts)} 块）：{os.path.basename(out_path)}")
        return out_path
    for p in parts:
        try:
            os.remove(p)
...
def download_iiif_fallback(net, manifest, item_id):
    ...
    if not img_urls:
        return None
    log(f"  [回退 IIIF] 下载 {len(img_urls)} 页图像并合成 PDF...")
    images = []
...
def download_item_images(net, item_id, out_dir, fmt='jpeg', force=False, idx=None, total=None):
    ...
    log(f"  下载 {npages} 页图片（{'JPEG2000 转换' if fmt == 'jp2' else 'JPEG'}）...")
    for i, u in enumerate(img_urls, 1):
        r = net.get(u)
        if r is None or r.status_code != 200:
            err(f"  第 {i} 页下载失败")
        ...
        if fmt == 'jp2':
            try:
                ...
                im.save(os.path.join(vol_dir, f"{i:04d}.jp2"), format='JPEG2000')
            except Exception as e:
                err(f"  第 {i} 页转换 JP2 失败: {e}")
                return None
        else:
            with open(os.path.join(vol_dir, f"{i:04d}.jpg"), "wb") as f:
                f.write(r.content)
        if i % 10 == 0:
            log(f"    已下载 {i}/{npages} 页")
...
def ask_download_scope(total):
    log(f"\n该古籍共 {total} 卷，请选择下载范围：")
    log("  1) 全部下载")
    log("  2) 仅下载前 N 卷")
    log("  3) 跳过此古籍")
    while True:
        try:
            choice = input("请选择 [1/2/3]（默认1全部）: ").strip()
        ...
        if choice == '2':
            while True:
                try:
                    s = input(f"请输入要下载的卷数 (1-{total}): ").strip()
                except (EOFError, KeyboardInterrupt):
                    log("\n  （非交互输入，默认全部下载）")
                    return 0
                if s.isdigit() and 1 <= int(s) <= total:
                    return int(s)
                err(f"  请输入 1-{total} 之间的数字")
        else:
            err("  请输入 1、2 或 3")
...
def ask_download_format():
    log("\n请选择下载格式：")
    log("  1) PDF（合并为单个 PDF 文件）")
    log("  2) JPEG（每页存为单独 jpg 图片，存入卷子文件夹）")
    log("  3) JPEG2000（每页存为单独 jp2 图片，从 JPEG 本地转换）")
    while True:
        try:
            choice = input("请选择 [1/2/3]（默认1 PDF）: ").strip()
        except (EOFError, KeyboardInterrupt):
            log("\n  （非交互输入，默认 PDF）")
            return 'pdf'
        if choice in ('1', ''):
...
def process_one(net, url, out_dir, limit, force, max_volumes=0, fmt='pdf'):
    ...
    if meta.is_book:
        log(f"■ 簿冊（多卷书）：{meta.title}  ID={item_id}")
        ids, total = find_book_items(net, item_id)
        if not ids:
            log(f"  未找到子件名，按单件下载封面/内容...")
            if fmt == 'pdf':
    ...
    if total and total > 1:
        ...
        log(f"■ 件名系列：{subject or meta.title}  ID={item_id}（全 {total} 点中）")
        if max_volumes:
            n = max_volumes
        else:
            ...
            if n == -1:
                log("  已跳过")
                return
        scope = f"前 {n} 卷" if n else "全部"
        log(f"  {scope}（{total} 点中）")
        if first_id and first_id != item_id:
            log(f"  当前件名 {item_id} 非首件，从先頭件名 {first_id} 开始枚举...")
            start_id = first_id
            start_next, _, _, _ = _fetch_nav_fast(net, start_id)
    ...
    log(f"■ 件名（单卷/单图）：{meta.title}  ID={item_id}")
    if fmt == 'pdf':
        download_item_pdf(net, item_id, out_dir, limit, force)
    else:
        download_item_images(net, item_id, out_dir, fmt, force)
